...
int main(int argc, char** argv) {
  ...
  // load image
  IplImage* img = cvLoadImage(argv[5], CV_LOAD_IMAGE_GRAYSCALE);
  if (!img){
  ...
  printf("\n");
  // load photo
  img = cvLoadImage(argv[6], 1);
  printf("width=%d, height=%d\n", img->width, img->height);
  
  // cut photo and classify
  for(int i=0; i<locate_x.size()-1; i++){
    for(int j=0; j<locate_y.size()-1; j++){
	int C = locate_x[j+1]-locate_x[j], R = locate_y[i+1]-locate_y[i];
	//printf("Now = (%d,%d) %d*%d\n", locate_x[j], locate_y[i], C, R);
	IplImage* snapshot = cvCreateImage( cvSize(C, R), 8, 3 );
	cvSetImageROI( img, cvRect( locate_x[j], locate_y[i], C, R ) );
    	cvCopy( img, snapshot );
	//cvShowImage("PICTURE", snapshot);
	std::vector<Prediction> predictions = classifier.Classify(snapshot);
	for (size_t k = 0; k < predictions.size(); ++k){
	  Prediction p = predictions[i];
	  p.first = p.first.substr(11, p.first.length()-1);
	  std::cout << p.first << " ";
	}
    }
    cout << endl;
  ...
  cv::Rect roi;
  roi.width = img.size().width/w;
  roi.height = img.size().height/h;
  for (int j=0; j<w; j++){
    for (int k=0; k<h; k++){
      roi.x = roi.width * j;
      roi.y = roi.height * k;
      ...
      for (size_t i = 0; i < predictions.size(); ++i) {
        ...
        //      << p.first << "\"" << std::endl;
        pre = Convert(p.second);
	nam = p.first;
	//cout << std::fixed << std::setprecision(4) << pre << '-' << nam << endl;
	putText(img, pre, Point(roi.x+20, roi.y+20), FONT_HERSHEY_COMPLEX,	0.5, c);
	putText(img, nam, Point(roi.x+20, roi.y+40), FONT_HERSHEY_COMPLEX,	0.5, c);
      }
    }
  }
  ...
  */
  ...
  std::cout << "Total time: " << duration << endl;
}
#else
int main(int argc, char** argv) {
